...
router.post('/reprocess-bulk', async (req, res) => {
  try {
    ...
    const result = await reprocessBulkItems(filterAll)
    res.json(result)
  } catch (err) {
    logger.error(err, 'POST /api/items/reprocess-bulk error')
    res.status(500).json({ error: 'Failed to reprocess items' })
  }
})
...
router.get('/reminders/due', async (_req, res) => {
  try {
    const sql = buildItemQuery({
      where: 'i.deleted_at IS NULL AND i.remind_at IS NOT NULL AND i.remind_at <= NOW()',
      orderBy: 'i.remind_at ASC'
    })
    const { rows } = await pool.query<ItemRow>(sql)
...
router.get('/export/obsidian', async (_req, res) => {
  try {
    const sql = buildItemQuery({
      where: 'i.deleted_at IS NULL AND i.encrypted = FALSE',
      orderBy: 'i.created_at DESC'
    })
    const { rows } = await pool.query<ItemRow>(sql)
...
router.get('/:id/related', async (req, res) => {
  try {
    ...
    const sql = buildItemQuery({
      ...
      orderBy: 'i.embedding <=> $2::vector',
      limit: '5'
    })
    const { rows } = await pool.query<ItemRow>(sql, [req.params.id, JSON.stringify(embedding)])
    res.json(rows.map(rowToItem))
  } catch (err) {
